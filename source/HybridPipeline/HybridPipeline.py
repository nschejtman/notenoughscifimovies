...
print (fslim.W_sparse[:, 6] != 0).sum(), (fslim.W_sparse.data > 1e-4).sum()
ranking_fslim = fslim.predict(urm_explicit, 5, non_active_items_mask)
ranking_fslim = np.array(ranking_fslim).flatten()
np.save("ranking5 fslim", ranking_fslim)
# ranking_fslim = np.load("rnking5 fslim.npy")
...
save_sparse_matrix(urm_explicit, "enriched")'''
...
print (scm.W_sparse[:,6] != 0).sum(), (scm.W_sparse.data > 1e-4).sum()
ranking_scm = scm.predict(urm_pred, 5, non_active_items_mask)
ut.write_recommendations("HybridPipeline fSLIM 30000k 100000alpha 2000sh implrating CBsim SCM SVM 1minus2", ranking_scm, test_users_idx, item_ids)
