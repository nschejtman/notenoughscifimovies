...
def cv_search(rec, urm, fslim_sim_mat, non_active_items_mask, sample_size, sample_from_urm=True):
    np.random.seed(1)
    ...
    params = {'alpha_ridge':[10000, 25000, 50000, 75000, 100000], 'k_nn':[20000]}
    ...
    for pars in grid:
        ...
        for row_train, row_test in splits:
            ...
            rec.fit(urm_train, fslim_sim_mat)
...
class HybridSsfSp(BaseEstimator):
    """
...
def get_knn(sim_mat, i, k):
    sims = sim_mat[:,i].toarray().ravel()
    top_k = np.argsort(sims).flatten()
    return top_k[-k:]
